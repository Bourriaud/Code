  ...
  subroutine interp(mesh,sol,order,gauss_weight,gauss_point,sol_refine,sol_interp)
    ...
    do k=1,mesh%nc
       ...
       case(1)
          ...
          mesh%cell(k)%polCoef=mesh%cell(k)%polMax
          xc=mesh%cell(k)%xc
          yc=mesh%cell(k)%yc
          ...
          do p1=1,order
             do p2=1,order
                ...
                call evaluate(mesh,sol,k,order,gauss_weight,Xg,Yg,u)
                sol_interp(k,:)=sol_interp(k,:)+u*gauss_weight(p1)*gauss_weight(p2)/4.0_dp
                Xg=xc+dx+gauss_point(p1)*dx
                Yg=yc-dy+gauss_point(p2)*dy
                call evaluate(mesh,sol,k,order,gauss_weight,Xg,Yg,u)
                sol_interp(k+mesh%nc,:)=sol_interp(k+mesh%nc,:)+u*gauss_weight(p1)*gauss_weight(p2)/4.0_dp
                Xg=xc-dx+gauss_point(p1)*dx
                Yg=yc+dy+gauss_point(p2)*dy
                call evaluate(mesh,sol,k,order,gauss_weight,Xg,Yg,u)
                sol_interp(k+2*mesh%nc,:)=sol_interp(k+2*mesh%nc,:)+u*gauss_weight(p1)*gauss_weight(p2)/4.0_dp
                Xg=xc+dx+gauss_point(p1)*dx
                Yg=yc+dy+gauss_point(p2)*dy
                call evaluate(mesh,sol,k,order,gauss_weight,Xg,Yg,u)
                sol_interp(k+3*mesh%nc,:)=sol_interp(k+3*mesh%nc,:)+u*gauss_weight(p1)*gauss_weight(p2)/4.0_dp
             enddo
          enddo
       end select
    enddo
